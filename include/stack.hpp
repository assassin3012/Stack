...
auto bitset::test(size_t index) const -> bool {
	if (index >= size_) throw::out_of_range("In test");
	else {
		return bit_[index];
	}
}
...
auto bitset::set(size_t index) -> void {
	if (index >= size_) throw::out_of_range("In set");
	else {
		bit_[index] = true;
	}
}
...
auto bitset::reset(size_t index) -> void {
	if (index >= size_) throw::out_of_range("In reset");
	else {
		bit_[index] = false;
	}
}
...
auto allocator<T>::destroy(T * ptr) -> void {
	if (ptr >= ptr_ + size_ || ptr < ptr_) throw::out_of_range("In destroy");
	size_t t = ptr - ptr_;
	if (bs_.test(t)) {
		ptr->~T();
		bs_.reset(t);
...
auto allocator<T>::construct(T * ptr, T const & value) -> void {
	if (ptr >= ptr_ + size_ || ptr < ptr_) throw::out_of_range("In construct");
	size_t t = ptr - ptr_;
	if (bs_.test(t)) {
		this->destroy(ptr);
	}
...
template <typename T>
void stack<T>::pop() {
	if (al_.empty()) throw::logic_error("In pop");
	else {
...
const T& stack<T>::top() const {
	if (al_.empty()) throw::logic_error("In top");
	else {
